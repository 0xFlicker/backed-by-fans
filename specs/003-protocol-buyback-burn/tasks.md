...
Assessment: 2026-09-09. The standing-settings replacement governs superseded economic-policy requirements. Subsequent user-approved stateless rehearsal and one-button Burn behavior remain authorized; these tasks do not restore expiring policies, required price signatures, child-fork rehearsal or a production scheduler. Existing completion markers above remain historical. Complete T096 and T097 before the final acceptance runs in T098; The user requested T099 documentation before acceptance runs; T098 must update recorded outcomes after execution. Preserve the user's running manual environment while preparing isolated acceptance runs.

- [ ] T096 Make `web/scripts/run-buybacks.ts` perform bounded useful work without aborting all execution when the deployment exceeds 1,000 tiers or 5,000 historical memberships; process already released inventory independently of completing collection discovery, and keep an unavailable tier from blocking unrelated eligible work. Use bounded traversal with explicit coverage/progress semantics suitable for independent one-shot invocations, retaining viem-owned submission/receipt handling and current gas preferences. Add regressions in `web/scripts/run-buybacks.test.ts` for populations above both existing caps, later-page earned fees, an unavailable tier, and ready vault funds despite incomplete collection; verify repeated invocations make progress without a persistent daemon or an unbounded scan per FR-019, FR-040, plan §5 collector traversal and T090 (partial; HIGH; F1).
- [ ] T097 Extend `scripts/protocol-fork/export-evidence.ts`, `scripts/protocol-fork/verify-evidence.ts`, `scripts/protocol-fork/manifest.schema.json` and their focused tests to include the factory-discovered immutable burn router, its factory/vault bindings, runtime hash, creating transaction and deployment measurements. Require retained batch execution evidence from `web/tests/e2e/protocol-burn.spec.ts`, reconcile receipt purchases and burns with actual supply changes, and add negative checks rejecting absent or mismatched router identity and batch evidence; integrate the new scenario into the acceptance matrix per FR-034, T077 and T078 (partial; HIGH; F2).
- [ ] T098 After the runner and evidence changes, execute the required local verification and two clean complete authentic acceptance runs against the final source snapshot using `scripts/verify-local.sh` and `scripts/test-protocol-fork.sh run`; adapt affected test expectations only to the approved standing-settings/stateless-rehearsal behavior. Retain and independently reconcile the full membership, Safe, runner replacement, bonding/graduation, multi-asset, compensation, browser and Burn outcomes, deployment limits and source identities through `scripts/protocol-fork/verify-evidence.ts`. Record failures as outstanding and keep historical policy evidence separate; do not substitute focused checks for the complete acceptance pair per FR-033–FR-035, SC-008 and T079–T085 (partial; HIGH; F3).
- [X] T099 Update `specs/003-protocol-buyback-burn/quickstart.md`, `contracts/operations-and-evidence.md`, `contracts/local-policy-tools.md` and the current implementation evidence entry points so operators have one consistent workflow for fresh setup versus state restoration, wallet funding and seeded memberships, stateless `eth_simulateV1` rehearsal, standing settings, the permissionless Burn action and bounded one-shot execution. Clearly label dated handoffs and superseded child-fork instructions as historical, retain the recorded source/origin scope of prior evidence, and document that fresh deployments change membership URLs. Preserve existing tasks and approved scope; do not reintroduce daily policies or a production Cron requirement per FR-034, T081 and the superseded rehearsal description in T090 (partial; MEDIUM; F4).


T099 documentation completed 2026-09-09 before final acceptance, as requested.
Quickstart and operator instructions were checked against the installed CLI help,
lifecycle and administration sources; shell examples passed `bash -n`, the
embedded state-export Python compiled, and operational relative file links resolve.
This pass did not run the export/restore round-trip, sign transactions, reset the
