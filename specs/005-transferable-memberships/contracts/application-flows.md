...
Preserve the established artwork-led account layout. Show current claimable totals by currency, with available USD estimates, beside a single **Claim all** action. Membership cards show a subtle token ID, expiration, and current claim amounts. Keep same-tier positions distinct. Derive cards and totals from the same current claim preview; do not display stored accounting balances as the amount to claim. Include ended-membership, referral, and creator rewards once per tier. Omit selection controls, empty reward panels, snapshot metadata, and accounting jargon from the normal account view; preserve fractional credit without presenting it as a claimable whole unit.

**Claim all** discovers the invocation’s positions and beneficiary balances automatically, sizes explicit batches by simulation/gas estimation, and reports completed and remaining work. The account page has no manual claim selection. Each tier’s beneficiary categories are accounted for correctly across split batches; new arrivals and continuous accrual do not extend the invocation indefinitely. Resource failures cause smaller batches; business failures remain explicit. Wallet rejection stops the sequence with successful receipts preserved for in-memory resumption. Recheck captured positions after transfers or burns before submitting each batch. A zero-NFT account claims its ended-membership balance through **Claim all**. Incomplete projection is never treated as zero or a final claimable amount.
...
Show a live-data counterpart to the About payment diagram, using the existing typography and open layout. Group by payment currency without adding incompatible token amounts. Show membership payments received and refunds, followed by creator/member/referral/protocol branches with still-accruing, earned-unclaimed, and cumulative-paid amounts. For the protocol branch, show Spent on buybacks from the membership-funded vault's cumulative spending and Available for buybacks as its unspent inventory plus earned protocol fees. Release happens within the combined buyback action and is not a separate availability category. Read vault inventory at the same block as earned fees and count it once per currency across discovery pages; exclude donation inventory from this membership-payment flow. Link to the vault's existing spending/burn report. Public totals include all positions, regardless of the viewer's wallet.

Discover tiers in bounded pages pinned to one block. Additional pages progressively advance discovery; label partial discovery or incomplete accounting explicitly. Failed reads retain the previous snapshot with an error rather than substituting zero. Refresh existing figures in place. On narrow screens, stack the branches in reading order and retain keyboard-operable currency selection.

Animate values with the existing rolling amount component: earned amounts count up, still-accruing amounts count down, and receipts/refunds/payouts animate when fresh snapshots change them. Sum fractional balances before rounding. Stop projection at the first funding or expiration boundary and after 30 seconds without a new snapshot; freeze incomplete or failed reads. Honor reduced-motion preferences without suppressing live numeric updates. Counters must not shift the surrounding layout.
