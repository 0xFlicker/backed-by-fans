...
- [ ] T074 **DEFERRED — user-approved; not a release blocker for this pass; not verified.** Run and record a named browser/screen-reader session for SC-012 in `specs/004-vesting-reward-curves/evidence/implementation.md`, following `specs/004-vesting-reward-curves/contracts/frontend.md`; verify actual configured summary values, custom input/errors, focus, claims/catch-up announcements, default animation and visual preferences. Fix observed feature-flow gaps in their owning components; automated semantic tests alone do not close this task.
...
Assessment: current source and recorded evidence against FR-001–057, SC-001–012, 38 story acceptance scenarios, 13 plan decision groups and five constitutional principles. Later approved revisions supersede earlier partial-success and settled-only designs. Source/test inspection is not a fresh full-suite execution or public deployment approval. Existing open acceptance entries remain unchanged; T100/T101 below close their specifically identified outstanding evidence rather than requiring duplicate rehearsals. Subsequent user instructions require using the existing local RPC 18557/web 3110 when rehearsal is authorized, without additional fork/web ports.

- [X] T097 HIGH Reconcile account-wide claim success from the successful receipt's registered-tier member/referral/creator payout events in `web/src/features/membership/AccountRewards.tsx`, reusing the existing payout reconciliation helpers; aggregate and show actual paid amounts by currency, validate beneficiary and emitting tiers, and preserve subsequently accruing balances. Add multi-tier/multi-currency and mismatched-event regression coverage in `AccountRewards.test.tsx`. Do not substitute preview estimates for paid amounts. Per FR-041, US6/AC4 and plan: account-wide settlement and claims (partial; F1).
- [X] T098 HIGH Decode submission-time `ClaimAccountingBehind(batchIndex,tier,accountedThrough,nextCheckpoint)` and `ClaimFailed(batchIndex,tier,reason)` in `web/src/features/membership/AccountRewards.tsx` and `web/src/lib/transaction-state.ts`; identify the affected membership, refresh authoritative previews, and offer the named bounded advance/retry action when accounting became behind after the last read. Preserve underlying failure context without dumping raw ABI errors or automatically changing/signing transaction intent. Test a boundary crossed between preview and simulation, contextual transfer failure, and successful retry. Per FR-043, FR-057 and plan: account-wide settlement and claims (partial; F2).
- [X] T099 MEDIUM Remove routine accruing balances from the account rewards live region in `web/src/features/membership/AccountRewards.tsx`; keep meaningful transaction, error and catch-up announcements separate from authoritative 15-second amount refreshes and animated ticks. Add regression coverage ensuring both preview refreshes and streaming updates do not change live-region content while action outcomes remain announced. Per FR-052 and contracts/frontend.md: statuses and recovery (contradicts; F3).
- [X] T100 MEDIUM Complete the existing pending account-wide browser rehearsal with actual `claimEverything` submission across multiple memberships/currencies and member/referral/creator entitlements, plus named accounting catch-up and retry. Extend `web/tests/e2e/accounting-preview.spec.ts` or a focused account-claims journey, compare receipt-paid amounts and canonical balances, and record exact source/build identity, local graph, commands, receipts and limitations in `specs/004-vesting-reward-curves/evidence/account-claims.md`. The current browser test only executes membership `claimAll`; button availability and contract unit tests do not close this evidence. Per SC-008, SC-010 and tasks: account-wide claims rehearsal (partial; F4).
- [ ] T101 **DEFERRED — user-approved; not a release blocker for this pass; not verified.** MEDIUM Complete and record the still-open named browser/screen-reader acceptance tracked by T074/T089, covering preset/custom publication, correction, claims, catch-up/retry and non-repeating accrual announcements; identify browser/assistive-technology versions and observed limitations in `specs/004-vesting-reward-curves/evidence/implementation.md`. Reuse one observed session to satisfy the existing entries; do not infer completion from accessibility-tree or automated tests. Per SC-012 and T074/T089 (partial; F5).
- [X] T102 MEDIUM Verify the final `web/src/components/StreamingAmount.tsx` and `web/src/app/globals.css` digit-roll implementation in the browser after the latest alignment/trailing-zero/three-meaningful-digit fixes. Cover rapid ticks, unchanged leading digits and decimal position, precision transitions, carries, downward corrections, localized separators, mobile layout, default motion, reduced motion and forced colors; strengthen `web/tests/e2e/streaming-earnings.spec.ts` beyond checking that an amount changed, run relevant current-source checks and append scoped evidence to `specs/004-vesting-reward-curves/evidence/streaming-earnings.md`. Distinguish historical isolated-run evidence from the subsequently authorized replacement on the original ports; do not create alternate services merely to reproduce the old evidence instructions. Per FR-052–054, SC-010 and tasks: streaming earnings final visual verification (partial; F6).


### Phase 13 resolution evidence

T097–T099: account confirmations now aggregate registered-tier payout events by payment currency and beneficiary; contextual factory errors trigger canonical preview refresh and identify the affected tier; routine balances are outside live regions while action outcomes remain announced. Focused account/error tests cover receipt values differing from previews, multiple categories/currencies, beneficiary mismatch, no-op claims, accounting races and token revert reasons.

T100: user explicitly reports three successful **Claim everything** transactions in the current browser review. Accepted as user-confirmed functional rehearsal, not agent-observed receipt replay or proof of every error scenario. Automated regressions separately cover named catch-up, intent changes, and failed submissions. No duplicate claim transaction is required to reproduce the user's confirmation.

T102: user explicitly confirmed the final visual animation item as done. Accepted as user visual validation; previous automated browser artifacts retain their original scope and source timing. The later source changes in this pass affect claims/status announcements, not the digit animation.

T101/T074/T089 remain the same unrecorded named assistive-technology session. User claim and visual confirmations do not establish screen-reader acceptance. No screen-reader observation is claimed.


### User-approved accessibility deferral

The user explicitly deferred the full named screen-reader acceptance session. T074, T089 and T101 describe the same deferred validation and remain unchecked because no session was performed. This deferral supersedes their earlier blocking acceptance language for this pass; it does not establish that SC-012 screen-reader acceptance passed. Routine accrual announcements were fixed and focused automated checks passed.
