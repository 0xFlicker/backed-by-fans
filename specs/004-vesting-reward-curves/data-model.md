...
| Funded lot | tokenId/generation/index, absolute start/end, raw gross, four raw allocations, original referrer, cumulative gross/allocation prefixes | Only positive gross creates a lot. Ordered nonoverlapping paid intervals. Future start may follow a zero-value gap. Referral identity fixed at payment. Four allocations sum exactly to gross. |
| Scheduled node | tokenId, generation, lot index, timestamp, START/END; heap position lookup | At most one node per member. Keys ordered timestamp then tokenId. Generation/head identity must match live state. Removal O(log M); no dangling canceled endpoint. |
