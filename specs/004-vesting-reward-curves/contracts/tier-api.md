...
| `previewAccounting(uint256 tokenId, address referrer, uint256 maxSteps)` | `AccountingPreview`: stored and projected `EarnedBalances`, `asOf`, checkpoint count and four scaled allocation deltas. Reads 0–256 checkpoints in memory. Check `current.status.complete`; partial values are through the returned cursor. No writes or token transfers. |
