...
export async function main() {
  ...
  // Economic authority is explicitly the test Safe. This independently stated
  // launch-reserve reference is never generated by an ordinary runner spot quote.
  ...
  const quoteReserve = await client.readContract({
    address: curve,
  ...
  const batch = 10n ** 15n;
  const net = batch - (batch * feeBps) / 10000n;
  const reference = (net * tokenReserve) / (quoteReserve + net);
  const now = (await client.getBlock()).timestamp;
  await writeFile(
    resolve(evidenceDir, "launch-reserve-reference.json"),
    JSON.stringify(
      { quoteReserve, tokenReserve, feeBps, batch, reference },
      (_, x) => (typeof x === "bigint" ? x.toString() : x),
      2,
    ) + "\n",
  );
  const curveRate = {
    numeratorRaw: reference.toString(),
    denominatorRaw: batch.toString(),
    toleranceBps: 100,
  };
  // Explicitly reviewed pinned observations, chosen by the disposable Safe's
  // operator. The runner has no code path that creates or renews these prices.
  const usdRate = {
    numeratorRaw: "40216887404570",
    denominatorRaw: "100000",
    toleranceBps: 100,
  };
  const stockRate = {
    numeratorRaw: "478129",
    denominatorRaw: "1000000000000000",
    toleranceBps: 100,
  };
  const conversionReference = {
    originBlock: 57010735,
    originHash:
      "0xdfc65146f32cfd10afd9a620b68c3fc5d02077677ce06c46303e49e80f0a96cf",
    source: "../route-quotes-20260907.json",
    usdPool,
    amdPool,
    usdRate,
    stockRate,
  ...
  await writeFile(
    resolve(evidenceDir, "conversion-reference.json"),
    JSON.stringify(conversionReference, null, 2) + "\n",
  );
  for (const [asset, cap, budget, rates] of [
    [zeroAddress, batch, 10n ** 16n, [curveRate]],
    [weth, batch, 10n ** 16n, [curveRate]],
    [usdg, 2400000n, 24000000n, [usdRate, curveRate]],
    [amd, 5n * 10n ** 15n, 5n * 10n ** 16n, [stockRate, usdRate, curveRate]],
  ] as const) {
    await configure("policy", {
      asset,
      expectedRevisionRaw: "1",
      policy: {
        validAfterRaw: now.toString(),
        validUntilRaw: (now + 900n).toString(),
        batchCapRaw: cap.toString(),
        totalBudgetRaw: budget.toString(),
        rates,
      },
      evidence: {
        reference:
          asset === zeroAddress || asset === weth
            ? "launch-reserve-reference.json"
            : "conversion-reference.json + launch-reserve-reference.json",
        rationale:
          "Test Safe authorizes finite raw exposure from separately retained launch and market observations, with a 1% net-output tolerance; no market-value guarantee.",
      },
    });
  }
